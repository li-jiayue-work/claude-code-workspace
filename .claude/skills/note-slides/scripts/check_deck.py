#!/usr/bin/env python3
"""HTML 交付前机械检查：验证生成的 slides HTML 是否满足 P0 门。"""
import argparse
...
def main():
    ...
    for index, block in enumerate(slide_blocks, start=1):
        ...
        for heading in headings:
            ...
            if display_len > 34 and (stat_count or has_three_column):
                warnings.append(f"{label}: long heading combined with grid or stat blocks.")
        if stat_count > 1:
            warnings.append(f"{label}: multiple stat-num blocks. Use only for comparable metrics.")
        for quote in callouts + headings:
            quote_len = visual_length(quote)
            if quote_len > 72 and has_three_column:
                warnings.append(f"{label}: long quote above three-column block. Split into separate slides.")
            elif quote_len > 72 and re.search(r"class=[\"'][^\"']*\bcallout\b", block, flags=re.I):
                warnings.append(f"{label}: oversized quote. Use excerpt layout or split the quote.")
        if has_table:
    ...
    if re.search(r"text-align\s*:\s*right", css, flags=re.I):
        warnings.append("Right-aligned text detected.")
