#!/usr/bin/env python3
"""发布前打包核心文件，排除本地生成物。可选 --git 初始化仓库。"""
import argparse
import os
import re
import shutil
...
def main():
    ...
    out_dir = Path(args.output).resolve() if args.output else Path(
        tempfile.mkdtemp(prefix="note-slides-core.")
    )
    if out_dir.exists():
    ...
    violations = [
        file for file in packed
        if any(pattern.search(file) for pattern in BLOCKED)
    ]
    if violations:
    ...
    if args.git:
        ...
        run(["git", "config", "user.name", "Note Slides"], out_dir)
        run(["git", "config", "user.email", "note-slides@local.dev"], out_dir)
...
def should_skip(path):
    return (
        path.name in {".DS_Store", "__pycache__"}
        or path.suffix == ".pyc"
    )
